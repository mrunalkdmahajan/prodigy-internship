...
# Use LabelEncoder to convert 'Age' categories into numerical values
le = LabelEncoder()
...
# Verify the structure of the DataFrame and the existence of 'Age_group'
print(df.info())
print(df.head())
...
# Use LabelEncoder to convert 'Age' categories into numerical values
le = LabelEncoder()
df['Age'] = le.fit_transform(df['Age_group'])
...
sns.countplot(x='Age_group', data=df, ax=ax[0, 3])  
...
sns.countplot(x='Age_group', data=df, hue='Survived', ax=ax[0, 3])  
...
X = df[['Age', 'Sex', 'Pclass']]  # Features in the desired order
y = df['Survived']
...
plt.savefig('./task-02/decision_tree2.png')
